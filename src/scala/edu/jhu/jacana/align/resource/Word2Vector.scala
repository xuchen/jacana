...
class Word2Vec {
  ...
  def load(filename: String, limit: Integer = Int.MaxValue, normalize: Boolean = true): Unit = {
    ...
    println(filename + " contains " + numWords + " words with vector size " + vecSize)
...
/**
 * wrapper object to provide features and values
 * 
 ...
 *
 */
object Word2VecWrapper extends Loggable {
    
    var dbPath = FileManager.getResource("resources/word2vec/vectors.bin")
    ...
    def getAverageSimilarity(tokens1: Array[String], tokens2: Array[String]): Double = {
        println(tokens1.toList + "\t\t" + tokens2.toList)
    	val matrix = Array.ofDim[Double](tokens1.length, tokens2.length)
    	var i = 0; var j = 0;
    	while (i < tokens1.length) {
    	    j = 0
    	    while (j < tokens2.length) {
    	        matrix(i)(j) = getSimilarity(tokens1(i), tokens2(j))
    	        j += 1
    	    }
    	    i += 1
    	}
        ...
        return sum/tokens1.length
    }
    
    def getSimilarity(token1: String, token2: String): Double = {
        if (model == null)
            init()
        val retList = model.rank(token1.toLowerCase(), Set(token2.toLowerCase()))
        return if (retList.size == 1 && retList(0)._2 > 0) retList(0)._2 else 0
    }

	def main(args: Array[String]) {
	    println(Word2VecWrapper.getSimilarity("marry", "husband"))
	    println(Word2VecWrapper.getSimilarity("marry", "wife"))
	    println(Word2VecWrapper.getSimilarity("marry", "children"))
	    println(Word2VecWrapper.getSimilarity("marry", "rock"))
	    println(Word2VecWrapper.getSimilarity("marry", "jalsdjfl;ajsdf"))
	    println(Word2VecWrapper.getSimilarity("the", "the"))
	    println(Word2VecWrapper.getSimilarity("the", "wife"))
	    println(Word2VecWrapper.getSimilarity("wife", "husband"))
	    println(Word2VecWrapper.getAverageSimilarity(Array("the", "wife"), Array("the", "husband")))
