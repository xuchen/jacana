...
import edu.jhu.jacana.nlp.WordNetNotThreadsafe;
	...
	public double nonNormalizedTreeDist(LblTree t1, LblTree t2) {
		...
		if (q2a) {
			for (int j=1; j<nc2; j++) {
				...
				//HashSet<String> hypernyms = WordNetNotThreadsafe.getAllHypernyms(word2[j], pos2[j]);
				//HashSet<String> synonyms = WordNetNotThreadsafe.getAllSynonyms(word2[j], pos2[j]);
				//HashSet<String> hyponyms = WordNetNotThreadsafe.getAllHyponyms(word2[j], pos2[j]);
				HashSet<String> hypernyms = WordNetNotThreadsafe.getHypernymsSet(word2[j], pos2[j]);
				HashSet<String> synonyms = WordNetNotThreadsafe.getSynonymsSet(word2[j], pos2[j]);
				HashSet<String> hyponyms = WordNetNotThreadsafe.getHyponymsSet(word2[j], pos2[j]);
				HashSet<String> derived = WordNetNotThreadsafe.getDerivedSet(word2[j], pos2[j]);
				...
				if (pos2[j].startsWith("vb")) {
					entails = WordNetNotThreadsafe.getEntailingSet(word2[j]);
					causing = WordNetNotThreadsafe.getCausingSet(word2[j]);
				}
				if (pos2[j].startsWith("nn")) {
					membersOf = WordNetNotThreadsafe.getMembersOfSet(word2[j]);
					substancesOf = WordNetNotThreadsafe.getSubstancesOfSet(word2[j]);
					partsOf = WordNetNotThreadsafe.getPartsOfSet(word2[j]);
					HaveMember = WordNetNotThreadsafe.getHaveMemberSet(word2[j]);
					HaveSubstance = WordNetNotThreadsafe.getHaveSubstanceSet(word2[j]);
					HavePart = WordNetNotThreadsafe.getHavePartSet(word2[j]);
				}
				for (int i=1; i<nc1; i++) {
					if (lemma1[i].equals(lemma2[j]))
						canSubstitute[i][j] = true;
					...
					if ((hypernyms != null && hypernyms.contains(word1[i]))) {
						...
						if (entails != null && entails.contains(word1[i])) {
							...
							canSubstitute[i][j] = true;
							c_causing += 1;
						}  else if (WordNetNotThreadsafe.sameSynset(word1[i], pos1[i], word2[j], pos2[j])) {
							//subCost[i][j] +=  this.c_wn;
							c_sameSynset += 1;
						} else {
							subCost[i][j] +=  this.c_wn;
			...
			for (int i=1; i<nc1; i++) {
				...
				//HashSet<String> hypernyms = WordNetNotThreadsafe.getAllHypernyms(word1[i], pos1[i]);
				//HashSet<String> synonyms = WordNetNotThreadsafe.getAllSynonyms(word1[i], pos1[i]);
				//HashSet<String> hyponyms = WordNetNotThreadsafe.getAllHyponyms(word1[i], pos1[i]);
				HashSet<String> hypernyms = WordNetNotThreadsafe.getHypernymsSet(word1[i], pos1[i]);
				HashSet<String> synonyms = WordNetNotThreadsafe.getSynonymsSet(word1[i], pos1[i]);
				HashSet<String> hyponyms = WordNetNotThreadsafe.getHyponymsSet(word1[i], pos1[i]);
				HashSet<String> derived = WordNetNotThreadsafe.getDerivedSet(word1[i], pos1[i]);
				HashSet<String> entails = null;
				HashSet<String> causing = null;
				HashSet<String> membersOf = null;
				HashSet<String> HaveMember = null;
				...
				if (pos1[i].startsWith("vb")) {
					entails = WordNetNotThreadsafe.getEntailingSet(word1[i]);
					causing = WordNetNotThreadsafe.getCausingSet(word1[i]);
				}
				if (pos1[i].startsWith("nn")) {
					membersOf = WordNetNotThreadsafe.getMembersOfSet(word1[i]);
					substancesOf = WordNetNotThreadsafe.getSubstancesOfSet(word1[i]);
					partsOf = WordNetNotThreadsafe.getPartsOfSet(word1[i]);
					HaveMember = WordNetNotThreadsafe.getHaveMemberSet(word1[i]);
					HaveSubstance = WordNetNotThreadsafe.getHaveSubstanceSet(word1[i]);
					HavePart = WordNetNotThreadsafe.getHavePartSet(word1[i]);
				}
				for (int j=1; j<nc2; j++) {
					if (lemma1[i].equals(lemma2[j]))
						canSubstitute[i][j] = true;
