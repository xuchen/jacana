...
public class EditDistQuestion extends EditBasedDist {
	...
	public static HashSet<String> qWords = new HashSet<String> () {
		{add("who"); add("when"); add("what"); add("where");add("how"); add("which");}
		};
	public static HashSet<String> qPos = new HashSet<String> () {{add("cd"); add("nn"); add("nns"); add("nnp"); add("nnps");}};
	...
	HashSet<String> lemmas1 = new HashSet<String>();
	
	public String getQWord() {return qWord;}
	public String getAWord() {return aWord;}
	public ArrayList<Integer> getAWordList() {
		return aWordList;
	}
	...
	public EditDistQuestion(boolean normalized) {
		this(1, 1, 1, normalized);
	}
	
	public EditDistQuestion(double ins, double del, double update, boolean normalized) {
	...
	public double getUpdate(int i, int j) {
		...
		if (this.isDep)
			if (checkQueryAnswerMatch(i, j)) {
				this.qWord = computeQwords(i);
				this.aWord = computeAwords(j);
				return 0;
			} else if (lemma1[i].equals(lemma2[j]))
	...
	private boolean checkQueryAnswerMatch(int i, int j) {
		if (qWords.contains(lemma1[i]) && !lemmas1.contains(lemma2[j]) && qPos.contains(pos2[j])) {
			if (lemma1[i].equals("how")) {
				LblTree node = id2node1.get(i);
				int idx = node.getIdxInWordOrder();
				int idxNextWord = idx+1;
				int idPostOrderNextWord = (Integer)this.idxInWordOrder2node1.get(idxNextWord).getIdxInPostOrder();
				if (!pos1[idPostOrderNextWord].startsWith("vb") && pos2[j].startsWith("cd")) {
					// it's not a "how was/is/does/did... ..." question
					// and the answer phrase is a number
					return true;
				} else if (pos1[idPostOrderNextWord].startsWith("vb"))
					return true;
				else return false;
			}
			if (lemma1[i].equals("when")) {
				if (pos2[j].startsWith("cd")) return true;
				else return false;
			}
			return true;
		} else	return false;
	...
	private String computeQwords(int i) {
		// how often, when, how many, how much, how, how long, how old, how far, how large, how often
		if (lemma1[i].equals("how")) {
			LblTree node = id2node1.get(i);
			int idx = node.getIdxInWordOrder();
			int idxNextWord = idx+1;
			int idPostOrderNextWord = (Integer)this.idxInWordOrder2node1.get(idxNextWord).getIdxInPostOrder();
			if (pos1[idPostOrderNextWord].startsWith("vb")) {
				// questions such as "how was/is ..." not asking for a number
				return lemma1[i];
			} else {
				return lemma1[i] + " " + lemma1[idPostOrderNextWord];
			}
		} else
			return lemma1[i];
	}
	...
	/**
	 * return the subtree with node j as the parent
	 */
	private String computeAwords(int j) {
		LblTree node = this.id2node2.get(j);
		int[] list = new int[1+node.getChildCount()];
		ArrayList<Integer> aWordList = new ArrayList<Integer>(); 
		int i = 0;
		list[i++] = node.getIdxInWordOrder();
		for (Enumeration e = node.children(); e.hasMoreElements();) {
			LblTree child = (LblTree)e.nextElement();
			list[i++] = child.getIdxInWordOrder();
		}
		Arrays.sort(list);
		StringBuilder sb = new StringBuilder();
		for (int k:list) {
			int l = this.idxInWordOrder2node2.get(k).getIdxInPostOrder();
			if (k > 0 && !rel2[l].equals("p") && !pos2[l].equals("in")) {
				// not root, not punctuation
				sb.append(word2[l]+" ");
				aWordList.add(k);
			}
		}
		this.aWordList = aWordList;
		...
		for (int i1 = l1[i]; i1 <= i; i1++) {
			...
			for (int j1 = l2[j]; j1 <= j; j1++) {
				...
				if ((l1[i1] == l1[i]) && (l2[j1] == l2[j])) {
					...
					// however, after testing, this order was difficlut to get the following case right:
...
//						insert ck , ck+1 , . . . , cm as children of the new node v (preserving order)
					...
					if (cUpd < min) {
						...
						if (cUpd - forestdist[i1-1][j1-1] != 0.0 && !lbl1[i1].equals(lbl2[j1]) && 
							...
							// identical nodes, renaming cost is 0
							edits[i1][j1] = new Edit(Edit.TYPE.NONE, null);
							if (checkQueryAnswerMatch(i1, j1)) {
								this.qWord = this.computeQwords(i1);
								this.aWord = this.computeAwords(j1);
							}
						}
						this.align1to2.put(i1, j1);
						this.align2to1.put(j1, i1);
...
//				break;
	...
	public String printHumaneEditScript() {
		...
		StringBuilder sb = new StringBuilder();
		int i,j,k,l;
		for (int ii=this.compactEditList.size()-1; ii>=0; ii--) {
			Edit e = this.compactEditList.get(ii);
			switch (e.getType()) {
			case DEL:
			case DEL_LEAF:
			case DEL_SUBTREE:
