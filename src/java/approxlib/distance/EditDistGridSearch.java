...
/**
 * EditDist with various costs assigned by users. This has gone through
 * two revisions: At first (svn r149), it searched through the following relations of wordnet:
 * hypernym: 52
hyponym: 72
synonym: 1355
entails: 0
...
haveParts: 5
sameSynset: 1659
 * but only hypernym/hyponym/synonym/sameSynset fired often enough. Thus
 * in the second reivision, we only care about these relations, plus lemma match
 * and pp match
 * 
 * @author Xuchen Yao
 *
 */
public class EditDistGridSearch extends EditDist {
	
	// wordnet relation counter, to log stat of each one fired for the whole corpus
	private static int count_hypernym = 0, count_hyponym = 0, count_synonym = 0, count_sameSynset = 0;
	...
	public EditDistGridSearch(boolean normalized) {
		this(1, 1, 1, normalized);
		this.q2a = true;
	}
	
	public EditDistGridSearch(boolean normalized, boolean q2a) {
		this(1, 1, 1, normalized);
		this.q2a = q2a;
	}
	
	public EditDistGridSearch(boolean normalized, boolean q2a, float  c_pos, float c_dep, float c_lemma, 
			float c_hypernym, float c_hyponym, float c_synonym, float c_sameSynset, float c_pp) {
		this(normalized, q2a);
		this.c_pos = c_pos;
		this.c_dep = c_dep;
		this.c_lemma = c_lemma;
		this.c_hypernym = c_hypernym;
		this.c_hyponym = c_hyponym;
		this.c_synonym = c_synonym;
		this.c_sameSynset = c_sameSynset;
		this.c_pp = c_pp;
	}
	...
	private static HashSet<String> loadPPs() {
		...
		try {
			BufferedReader f = FileManager.getReader(FileManager.getResource("resources/paraphrase/unigram.pp.gz"));
			String line, pp1, pp2;
			String[] splits;
	...
	public EditDistGridSearch(double ins, double del, double update, boolean normalized) {
		...
		if (q2a) {
			for (int j=1; j<nc2; j++) {
				...
				for (int i=1; i<nc1; i++) {
					...
					if ((hypernyms != null && hypernyms.contains(word1[i]))) {
						...
						subCost[i][j] += c_hypernym;
					}
					if ((synonyms != null && synonyms.contains(word1[i]))) {
						canSubstitute[i][j] = true;
						count_synonym += 1;
					} else {
						subCost[i][j] += c_synonym;
					}
					if ( (hyponyms != null && hyponyms.contains(word1[i]))) {
						canSubstitute[i][j] = true;
						count_hyponym += 1;
						//subCost[i][j] +=  this.c_wn;
					} else {
							subCost[i][j] +=  this.c_hyponym;
					}
					if (WordNet.sameSynset(word1[i], pos1[i], word2[j], pos2[j])) {
						//subCost[i][j] +=  this.c_wn;
						count_sameSynset += 1;
					} else {
						subCost[i][j] +=  this.c_sameSynset;
					}
				}
			}
		} else {
			...
			for (int i=1; i<nc1; i++) {
				...
				for (int j=1; j<nc2; j++) {
					...
					if (synonyms != null && synonyms.contains(word2[j])) {
						canSubstitute[i][j] = true;
						count_synonym += 1;
					} else {
						subCost[i][j] += c_synonym;
					}
					if (hyponyms != null && hyponyms.contains(word2[j])) {
						canSubstitute[i][j] = true;
						count_hyponym += 1;
					} else {
						subCost[i][j] += c_hyponym;
					}
					if (WordNet.sameSynset(word1[i], pos1[i], word2[j], pos2[j])) {
						count_sameSynset += 1;
					} else {
...
//				break;
	...
	public static void printStat() {
		System.out.println(String.format("hypernym: %d\nhyponym: %d\nsynonym: %d\n" +
				"sameSynset: %d\n",
			count_hypernym, count_hyponym, count_synonym, count_sameSynset
		));
