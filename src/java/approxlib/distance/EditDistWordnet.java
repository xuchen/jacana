...
public class EditDistWordnet extends EditDist {
	...
	public EditDistWordnet(boolean normalized) {
		this(1, 1, 1, normalized);
		this.q2a = true;
	}
	
	public EditDistWordnet(boolean normalized, boolean q2a) {
		this(1, 1, 1, normalized);
		this.q2a = q2a;
	}
	
	public EditDistWordnet(double ins, double del, double update, boolean normalized) {
	...
	public double nonNormalizedTreeDist(LblTree t1, LblTree t2) {
		...
		for (int i=0; i<nc1; i++) {
			for (int j=0; j<nc2; j++) {
				...
				// BUG fix: forgot to add the following two lines
		...
		if (q2a) {
			for (int j=1; j<nc2; j++) {
				if (!pos2[j].startsWith("nn") && !pos2[j].startsWith("vb")) continue;
				...
				for (int i=1; i<nc1; i++) {
					...
					if (!pos1[i].startsWith("nn") && !pos1[i].startsWith("vb")) continue;
					if (!pos2[j].subSequence(0, 2).equals(pos1[i].subSequence(0, 2))) continue;
					if ((hypernyms != null && hypernyms.contains(word1[i])) || 
							(synonyms != null && synonyms.contains(word1[i])) || 
							(hyponyms != null && hyponyms.contains(word1[i]))) {
						canSubstitute[i][j] = true;
					} 
					else if (pos2[j].startsWith("vb")) {
						if (entails != null && entails.contains(word1[i]) || causing != null && causing.contains(word1[i]))
						canSubstitute[i][j] = true;
					} else if (pos2[j].startsWith("nn")) {
						if (membersOf != null && membersOf.contains(word1[i]) ||
								substancesOf != null && substancesOf.contains(word1[i]) || 
								partsOf != null && partsOf.contains(word1[i]) ||
								HaveMember != null && HaveMember.contains(word1[i]) ||
								HaveSubstance != null && HaveSubstance.contains(word1[i]) ||
								HavePart != null && HavePart.contains(word1[i]))
						canSubstitute[i][j] = true;
					}
				}
			}
			...
			for (int i=1; i<nc1; i++) {
				...
				for (int j=1; j<nc2; j++) {
					...
					if ((hypernyms != null && hypernyms.contains(word2[j])) || 
							(synonyms != null && synonyms.contains(word2[j])) || 
							(hyponyms != null && hyponyms.contains(word2[j]))) {
						canSubstitute[i][j] = true;
					} 
					else if (pos1[i].startsWith("vb")) {
						if (entails != null && entails.contains(word2[j]) || causing != null && causing.contains(word2[j]))
						canSubstitute[i][j] = true;
					} else if (pos1[i].startsWith("nn")) {
						if (membersOf != null && membersOf.contains(word2[j]) ||
								substancesOf != null && substancesOf.contains(word2[j]) || 
								partsOf != null && partsOf.contains(word2[j]) ||
								HaveMember != null && HaveMember.contains(word2[j]) ||
								HaveSubstance != null && HaveSubstance.contains(word2[j]) ||
								HavePart != null && HavePart.contains(word2[j]))
						canSubstitute[i][j] = true;
					}
				}
			}
